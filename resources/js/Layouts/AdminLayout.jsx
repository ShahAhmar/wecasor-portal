...
export default function AdminLayout({ children, title, user }) {
    ...
    const hasAnyRole = (allowedRoles) => {
        ...
        if (allowedRoles.length === 0) return true; // Empty array means visible to all
        return auth.roles.some(role => allowedRoles.includes(role));
    };

    const navLinks = [
        { name: 'Dashboard', href: '/admin/dashboard', roles: [], icon: (
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"></path></svg>
        )},
        { name: 'Studies', href: '/admin/studies', roles: ['Super Admin', 'Study Admin', 'Country Lead'], icon: (
        ...
        { name: 'Document Vault', href: '/admin/documents', roles: ['Super Admin', 'Study Admin', 'Country Lead'], icon: (
        ...
        { name: 'Institutions', href: '/admin/institutions', roles: ['Super Admin', 'Country Lead'], icon: (
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"></path></svg>
        )},
        { name: 'Users', href: '/admin/users', roles: ['Super Admin'], icon: (
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"></path></svg>
        )},
        { name: 'Surveys Hub', href: '/admin/surveys', roles: ['Super Admin'], icon: (
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
        )},
        { name: 'Conference Abstracts', href: '/admin/conference/abstracts', roles: ['Super Admin', 'Study Admin'], icon: (
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
        )},
        { name: 'Event Registrations', href: '/admin/conference/registrations', roles: ['Super Admin', 'Study Admin'], icon: (
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"></path></svg>
        )},
    ];

    const systemLinks = [
        { name: 'Audit Log', href: '/admin/audit', roles: ['Super Admin', 'Monitor / Auditor'], icon: (
        ...
        { name: 'Profile', href: '/admin/profile', roles: [], icon: (
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path></svg>
        )},
        { name: 'Settings', href: '/admin/settings', roles: ['Super Admin'], icon: (
        ...
        <div className="min-h-screen bg-[#f8fafc] flex">
            ...
            >
                ...
                <div className="flex-1 overflow-y-auto py-8 px-4 space-y-8 scrollbar-hide">
                    ...
                    {/* System Section */}
                    {systemLinks.filter(link => hasAnyRole(link.roles)).length > 0 && (
                        <div>
                            <h4 className="px-4 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mb-6">System Repository</h4>
                            <nav className="space-y-1">
                                {systemLinks.filter(link => hasAnyRole(link.roles)).map((link) => (
                                    <Link 
                                        key={link.name} 
                                        href={link.href}
                                        className="flex items-center gap-3 px-4 py-3 rounded-2xl text-slate-600 font-bold text-sm hover:bg-slate-50 hover:text-emerald-600 transition-all group"
                            ...
                            {notificationsOpen && (
                                <div className="absolute right-0 mt-3 w-80 bg-white rounded-3xl shadow-[0_20px_50px_rgba(0,0,0,0.1)] border border-slate-100 overflow-hidden z-[100] transform origin-top-right transition-all">
                                    <div className="p-6 border-b border-slate-50 flex items-center justify-between">
                                        ...
                                        <span className="px-2 py-0.5 bg-rose-50 text-rose-500 text-[10px] font-black rounded-lg">{(auth?.unread_notifications || []).length} NEW</span>
                                    </div>
                                    <div className="p-4 space-y-2 max-h-80 overflow-y-auto custom-scrollbar">
                                        {(!auth?.unread_notifications || auth.unread_notifications.length === 0) ? (
                                            <div className="p-4 text-center">
                                                <p className="text-[11px] font-bold text-slate-400">All caught up!</p>
                                            </div>
                                        ) : (
                                            auth.unread_notifications.map(notification => (
                                                <div key={notification.id} className="p-4 bg-slate-50/50 rounded-2xl border border-slate-100/50">
                                                    <p className="text-[11px] font-bold text-slate-800 leading-relaxed">{notification.message || 'System Notification'}</p>
                                                    <p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest mt-2 italic">
                        ...
                        <div className="relative">
                            ...
                            >
                                <div className="text-right hidden sm:block">
                                    <p className="text-xs font-black text-slate-800 tracking-tight">{auth?.user?.name || 'User'}</p>
                                    <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">{auth?.roles?.[0] || 'Investigator'}</p>
                                </div>
